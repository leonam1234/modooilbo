...
> 정적 export(`out/`) + **Pages Functions(`functions/api/*`: 인증·댓글·뉴스레터·조회수·시세 · `functions/partners/*`: 폐지된 `/partners` 를 410 으로 응답) + D1(`modooilbo-members`) + KV + R2 이미지(`img.modooilbo.com` — Pages 2만 파일 한도 회피, `scripts/sync-stock-r2.mjs`)**.
