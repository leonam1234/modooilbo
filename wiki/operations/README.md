...
> **이 폴더의 목적** — 실운영 중인 모두일보의 **콘텐츠·신뢰·유통·수익·법무·운영**을 유지·고도화하기 위해 반드시 해야 하는 작업을 다음 작업자(사람·LLM 에이전트)가 바로 착수할 수 있도록 정리한 SSOT다. 일부 본문에는 프로토타입 시절의 과제가 남아 있으므로 현재 구현은 코드와 루트 위키 정본을 함께 확인한다.
>
...
6. [04-publishing-protocol](04-publishing-protocol.md) — **일일 기사 발행 규약**. 역할 분리·독립 리뷰·유수화님 명시 승인·publishedAt 정본·발행 후 수정·Preview/라이브 검증·색인 인계
...
| **3** | 댓글·커뮤니티 | [02](02-growth-and-revenue.md) | **O(Pages Functions·D1)** | 인증 |
...
- Cloudflare Pages Functions 기반 인증·댓글·뉴스레터·조회수 API와 D1·KV·R2 하이브리드가 운영 중이다.
- Cloudflare beacon 흔적: `src/app/layout.tsx`(→ [02](02-growth-and-revenue.md)에서 정식화).

## ⚠️ 전 항목 공통 불변식 (어기지 말 것)
정적 export 제약·SSR안전(렌더중 Date.now/Math.random 금지)·단일 리드·dark: 페어링·`queries.ts` 단일 출처 등은 [08-conventions](../08-conventions.md)를 따른다. 인증·댓글 같은 독립 API는 현재 Pages Functions로 제공한다. **Next.js SSR·ISR·Server Actions처럼 Next 서버 런타임이 필요해질 때만** `output:"export"`를 버리고 `@opennextjs/cloudflare`(Workers)로 승급한다. 경로는 [06-deployment §10](../06-deployment.md)을 따른다.
