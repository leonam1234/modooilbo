...
> 2026-09-01부터 자동 발행·자동 배포는 취소됐다. 유수화님 명시 승인 전 CMS·Git·빌드·발행·배포 금지.

## 1. 일일 발행 파이프라인 (품질 통과분만 출고, 최대 24편)

```
원고 생산              개발 및 배포 담당자          독립 리뷰 담당          유수화님
┌──────────────┐   ┌──────────────┐   ┌──────────────┐   ┌──────────────┐
│ 패키지 작성       │ → │ 1차 정적·동적 게이트│ → │ 기사별 독립 검수    │ → │ 명시 승인 또는 보류 │
└──────────────┘   └──────┬───────┘   └──────┬───────┘   └──────┬───────┘
                          └─ 리뷰 요청 → 결과 회수·수정·최종 게이트 ─┘
                                                               승인 뒤 CMS·기술 게이트
                                                               → Git → 빌드 → 배포
```
...
- 독립 리뷰 PASS와 최종 게이트 HOLD 0도 **발행 승인과 별개**다. 승인 범위가 드러나는 유수화님의 현재 명시 메시지가 있어야 다음 단계로 간다.
...
- 승인 뒤 CMS 변환 단계에서 실행하는 `npm run report:editorial`: 출처 20 + 취재·검증 25 + 독자 가치 20 + 완결성 15 + 투명성 10 + 발행 위생 10.
...
6. 개발 및 배포 담당자가 수정 반영본의 최종 정적·동적 게이트를 끝내고 PASS/HOLD·수정·미확인을 유수화님에게 보고한다.
7. 유수화님이 해당 패키지를 명시 승인한 뒤에만 CMS 변환, 80점 코드 게이트, Git, 빌드, 발행, 배포를 수행한다.
...
승인 뒤 기사 내용이 바뀌면 기존 승인의 대상과 실제 발행본이 달라진다. 변경 기사를 독립 리뷰와
최종 동적 게이트에 다시 보내고 재승인을 받는다. 기계적 변환 실패처럼 본문을 바꾸지 않는 기술
