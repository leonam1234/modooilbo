...
- 배포 전 실제 검수자가 원문과 기사 수치·자격·기한을 대조하고 `reviewedBy`·`reviewedAt`을 남긴다. 2026-09-16부터 승인된 AI 독립 검수는 정확히 `모두일보 독립 리뷰 담당` 역할명을 쓰며, 사람 검수는 등록 기자 실명을 쓴다. AI 검수를 유수화 등 사람 실명으로 대신 기록하지 않는다. 종전 ‘사람 검수 필드’는 이 실제 독립 검수 기록을 뜻하며 별도 사람 검수 완료를 가정하지 않는다.
...
등록 기자 또는 승인된 AI 독립 검수 역할이 검수를 마친 뒤 실제 주체 그대로 입력한다. 현재 파일 기반 CMS는 로그인 서명까지 증명하지 못하므로,
...
### 기사별 제외·보완 회신 의무 (2026-09-23 지시, 다음 인수분부터 상시 적용)

1차 게이트·최종 게이트·배포에서 발행 제외, HOLD, WAIT_SOURCE, WAIT_AUTH,
재인수 대기 또는 기술 컷이 발생하면 PASS 기사와 제외 기사를 분리해 총괄에 회신한다.
제외 건수만 보고하지 않고 **제외 기사마다** 아래 7개 항목을 반드시 남긴다.

1. 제목·slug
2. 컷 단계와 판정 상태
3. 정확한 컷 사유
4. 판정 근거 파일·명령·URL·확인시각(KST). 해당하지 않는 항목은 해당 없음,
   확인하지 못한 항목은 미확인으로 구분하고 실제 실패 출력·증거 경로를 보존한다.
5. 보완 가능/불가. 판단 근거가 부족하면 임의로 단정하지 않고 확인 필요와 필요한 증거를 적는다.
6. 구체적 보완 방법과 담당 주체
7. 재검수 통과 조건

- 기술·CMS·빌드·배포 문제는 개발 및 배포 담당자가 보완안을 직접 제시한다.
- 편집·사실·출처 문제는 영구 독립 리뷰 담당(`01a05a60-a035-7921-8154-7aa4a7024f31`)에게
  보완안을 요청하고, 받은 답을 기사별 회신에 붙여 총괄에 전달한다. 회신 전에는 답변 대기로
  표시하며 검토자의 결론이나 보완 가능 여부를 대신 만들어 쓰지 않는다.
- 보완 뒤에는 해당 기사에 필요한 독립 재검수·동적·기술 게이트를 다시 통과시킨다.
  보고 완료를 PASS 또는 보류 해제로 간주하지 않으며, 이미 READY인 다른 기사의 배포는 지연시키지 않는다.
