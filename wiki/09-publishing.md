...
- 승인 뒤 CMS 변환 단계에서 `npm run check:editorial`(`--strict`)을 차단 게이트로 실행한다. `npm run report:editorial`은 같은 점수표를 보여 주는 보고용이며 실패를 차단하지 않는다. 배점은 출처 20 + 취재·검증 25 + 독자 가치 20 + 완결성 15 + 투명성 10 + 발행 위생 10이다.
...
7. 유수화님이 해당 패키지를 명시 승인한 뒤에만 CMS 변환, 80점 코드 게이트와 빌드를 수행하고 생성물을 포함해 비-master 릴리스 브랜치에 Git 커밋한다.
