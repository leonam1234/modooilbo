...
> 일일 24편 발행 체제의 정본 문서. 2026-08-08 기준, 그동안 커밋 메시지에만 흩어져 있던 규약을 문서화했다.
> 코드 게이트(강제)는 `scripts/build-content.mjs`, 운영 파이프라인은 이 문서가 기준이다. 어긋나면 코드가 우선.

## 1. 일일 발행 파이프라인 (12카테고리 × 2 = 24편)

```
코덱스(외부 에이전트)                     Claude(발행 담당)
┌─────────────────────┐    ┌──────────────────────────────────┐
│ 24편 원고 패키지 작성    │ →  │ 발행 직전 동적 게이트 전수 재확인       │ → 교정/HOLD → git commit → deploy
│ 1차 검수: PASS /       │    │ (편당 ~8항목, 하루 130~190개:        │
│  조건부 PASS / HOLD    │    │  원출처 URL 재조회·API 게이트·드리프트)  │
└─────────────────────┘    └──────────────────────────────────┘
```

- **일일 편성 12종**: 종합 6(economy·society·world·culture·sports·opinion — `tech`는 동결) + 사업 6(grants·bids·startup·industry·labor·deals).
