...
- **2026-09-02 유수화님의 상시 자동 배포 승인을 적용한다.** 독립 리뷰와 최종 게이트가 PASS·HOLD 0이면 별도 승인 질문 없이 CMS 변환, Git, 빌드, Preview, Production, 라이브 검증까지 진행한다. HOLD 또는 사용자의 현재 중지 지시가 있으면 멈춘다. 거버넌스 정본은 `wiki/09-publishing.md`, 배포 정본은 `wiki/06-deployment.md`, 현재 실행 체크리스트는 `HANDOVER.md` §5다.
...
패키지 발행은 이 일반 루틴을 먼저 실행하지 않고, `HANDOVER.md` §5에 따라 독립 리뷰와 최종
게이트가 PASS·HOLD 0이면 CMS 변환·게이트·빌드 → Git 커밋 → Preview 엔진·뷰포트 4조합 →
Production → 라이브 재검증 → 색인 인계를 자동으로 이어서 수행합니다.
