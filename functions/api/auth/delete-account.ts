 ...
 *   users DELETE가 FOREIGN KEY 위반으로 터지고 탈퇴가 실패한다(조용히 넘어가지 않는다 —
 *   핸들러 바깥 catch가 한국어 503 JSON으로 표면화한다).
...
export async function onRequestPost(ctx: any): Promise<Response> {
  ...
  // D1 일시 장애·FK 위반이 Cloudflare 원시 500으로 새 나가지 않게 — comments/index.ts와 같은 규약.
  try {
