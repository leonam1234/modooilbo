/**
 * GET /api/auth/google/start — 구글 로그인 시작.
 * CSRF 방지 state 쿠키 + 구글 인가 페이지 리다이렉트. (간편 회원가입: callback에서 자동 가입)
 */
...
export async function onRequestGet(ctx: any): Promise<Response> {
  const id = ctx.env.GOOGLE_CLIENT_ID;
  if (!id) return new Response("구글 로그인이 아직 설정되지 않았습니다.", { status: 503 });
