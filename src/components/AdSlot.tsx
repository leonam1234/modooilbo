 ...
 *
 * ★ 반응형 모드로 돌아가지 말 것: 반응형(`data-ad-format="auto"`)에서는 구글이 자기가 계산한
 *   크기를 인라인 스타일(보고에 따르면 `!important`)로 덮어써 **컨테이너 예약(min-height)을
 *   무력화**한다 → 예약해도 점프한다. CLS 0은 **치수를 우리가 확정할 때만** 성립한다.
 *   고정 크기 모드에서 구글 스크립트가 실제로 하는 일(로컬 관측): 우리 CSS 치수를 그대로 읽어
 *   `width:728px; height:90px`처럼 **되받아 적는다**(!important 없음, 치수 변형 없음).
 *   ⚠️ 단 이 관측은 localhost라 실광고가 아니다(아래 '로컬 측정의 한계') — 만약 실도메인에서
 *   구글이 치수를 덮어쓴다면 대응책은 SIZE의 w/h에 `!` 를 붙이는 것(`!w-[728px]` = 스타일시트
 *   !important는 '일반 인라인'을 이긴다). 지금 안 붙인 이유: 관측상 덮어쓰지 않는데 미리
 *   강제하면 구글이 정당하게 다른 크기를 줄 때 광고가 잘려 되레 정책 위반(가림)이 된다.
 *
 * ⚠️ 로컬 측정의 한계(결론 금지): localhost에는 구글이 실광고를 주지 않는다(테스트·대체 광고가
 *    뜨거나 unfilled). 실제 크기·충전·최종 CLS는 **실도메인 배포 후에만** 확정할 수 있다.
 *    여기 주석의 수치는 '설계가 의도대로 자리를 잡는가'까지만 증명한다.
 *
