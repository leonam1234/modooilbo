...
| `summary` | ✅* | 검색결과·기사 상단에 먼저 보일 고유 요약. 핵심 사실을 40자 이상으로 쓰고 이해관계·광고 고지 문장을 섞지 않음 |
...
- 별도 제공 패키지의 `direct`는 `류주형 / 취재기자`와 실제 취재 증빙을 요구한다. `desk`는 책임 배정이 확인된 현행 등록 로스터 김영환·유승현·서영호·남동균·최은영·류주형의 정식 byline을 허용한다. 박유주는 신규 기사에 사용하지 않는다. 오피니언은 등록 책임 필자의 직접 검토·채택 전 HOLD하며 자동 실명 부여로 해제하지 않는다.

- 기사별 `<title>`, description, canonical, Open Graph, Twitter, `NewsArticle`, 대표 이미지는 위 frontmatter에서 공통 템플릿이 자동 생성한다. 원고 본문에 메타태그를 직접 쓰지 않는다.
- `summary`는 제목 아래 리드와 검색 설명문의 첫 문장이다. 이해관계 고지보다 먼저 독자가 기사 핵심을 이해할 수 있도록 쓴다.
- `## 취재·이해관계 안내`, `## 취재 이해관계 고지`, `## 이해관계 안내`, `## 관계사 고지` 구간은 사이트가 본문 순서를 유지한 채 보조 `aside` 안내로 렌더링한다. 검색 설명문·자동 3줄 요약 후보에서는 제외한다.
- 위 고지 제목은 포털이 기사 핵심 H2 바로가기처럼 오인하지 않도록 화면에서는 H2가 아닌 안내 라벨로 표시한다. 일반 주제 소제목인 `## 이해관계자`는 변환하지 않는다.
- 고지 내용 자체는 숨기거나 삭제하지 않는다. 광고 기사는 별도의 `sponsor` 표시와 상·하단 광고 고지를 그대로 유지한다.
