...
새 처분 기준이 차액을 자동으로 돌려주는 제도라는 해석은 경계해야 한다. 이번 개정문에서 확인되는 핵심은 숙박업자의 요금표 게시·준수 의무와 지자체의 행정처분 기준이다. 예약 취소, 차액 반환, 손해배상 같은 개별 소비자 구제의 성립 여부와 절차까지 이 시행규칙이 일괄 확정하지는 않는다. 가격 차이가 확인됐더라도 결제 취소나 환급이 자동으로 처리되는 것은 아니다.
