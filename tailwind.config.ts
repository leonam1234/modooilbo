  ...
  theme: {
    extend: {
      colors: {
        ...
        //
        // ⚠️ 대비(WCAG AA) 규약 — ink와 동일한 원리(2026-07-15 a11y 감사):
        // signal-600(#6b6b73)은 **라이트 전용**이다. 흰 배경 5.28:1✓이지만 다크 배경에선
        // ink-950 3.73 / ink-900 3.50 / signal-950 3.00으로 소형 텍스트 AA(4.5:1) 미달이다.
        // 한 토큰을 양쪽 겸용할 수 없으므로(ink 주석의 교집합 공집합 논증과 같은 구조)
        // 팔레트 값은 그대로 두고 **사용처에서 페어링**한다:
        //   `text-signal-600 dark:text-signal-400` (hover/group-hover도 dark: 변형을 함께)
        // 다크 쪽은 기존 signal-400(#9a9aa1)이 이미 전 다크 배경에서 AA를 통과하므로
        // 별도 단계를 새로 만들지 않는다(ink-950 7.06 / ink-900 6.62 / signal-950 5.67 — 전부 ✓).
        // 액센트 텍스트 대비율(흰 배경 / ink-950 / ink-900 / signal-950):
        //   signal-400 2.80✗ / 7.06✓ / 6.62✓ / 5.67✓   ← 다크 전용
        //   signal-600 5.28✓ / 3.73✗ / 3.50✗ / 3.00✗   ← 라이트 전용
        // 예외(미페어링·의도됨): 체크박스 액센트 `text-signal-600`(텍스트가 아닌 폼 컨트롤 색이고
        //   @tailwindcss/forms 미설치라 렌더에 영향 없음), 밑줄 표식 `after:bg-signal-600`·
        //   장식 점 `bg-signal-500`(비텍스트 3:1 기준 통과).
        signal: {
          50: "#f6f6f7",
          100: "#ececee",
          200: "#d9d9dd",
